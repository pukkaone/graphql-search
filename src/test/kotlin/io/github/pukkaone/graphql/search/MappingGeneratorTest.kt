...
class MappingGeneratorTest {
    ...
    private val documentTypeToMappingMap: Map<String, ObjectNode> = MappingGenerator.generate(
        ProtoSchema("test").typeDefinitionRegistry
    )
    ...
    @Test
    fun `when type is Byte then translate to byte`() {
        val mapping = documentTypeToMappingMap["WithByte"]
        assertThat(mapping).hasToString(
            """
            {"properties":{"field":{"type":"byte"}}}
            """.trimIndent()
        )
    }
    ...
    @Test
    fun `when type is Short then translate to short`() {
        val mapping = documentTypeToMappingMap["WithShort"]
        assertThat(mapping).hasToString(
            """
            {"properties":{"field":{"type":"short"}}}
            """.trimIndent()
        )
    }
