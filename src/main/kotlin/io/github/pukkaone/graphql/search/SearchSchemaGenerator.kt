...
private val DENY_GENERATION_FROM_OBJECT_TYPES =
    setOf("Aggregation", "Bucket", "Mutation", "PageInfo", "Query", "Subscription")
...
class SearchSchemaGenerator(
    ...
    private fun generateInputTypes() {
        ...
        for (objectTypeDefinition in typeDefinitionRegistry.getTypes(ObjectTypeDefinition::class.java)) {
            if (objectTypeDefinition.name in DENY_GENERATION_FROM_OBJECT_TYPES) {
                continue
            }
    ...
    private fun generateGroupByOutputType(objectTypeDefinition: ObjectTypeDefinition) {
        schemaBuilder.append(
            """

            type ${objectTypeDefinition.name}GroupBy {
        ...
        for (fieldDefinition in objectTypeDefinition.fieldDefinitions) {
            ...
            if (typeDefinitionRegistry.isObjectType(fieldType)) {
                schemaBuilder.append("  ${fieldDefinition.name}: ${fieldType.name}GroupBy\n")
            } else {
                schemaBuilder.append("  ${fieldDefinition.name}: [Bucket!]\n")
            }
        }
    ...
    private fun generateSearchOutputTypes(objectTypeDefinition: ObjectTypeDefinition) {
    ...
    private fun generateOutputTypes() {
        for (objectTypeDefinition in typeDefinitionRegistry.getTypes(ObjectTypeDefinition::class.java)) {
            if (objectTypeDefinition.name in DENY_GENERATION_FROM_OBJECT_TYPES) {
                continue
            }
            ...
            if (objectTypeDefinition.name != "GeoPoint") {
                generateGroupByOutputType(objectTypeDefinition)
            }

            if (DocumentDirective.isPresent(objectTypeDefinition)) {
                generateSearchOutputTypes(objectTypeDefinition)
            }
        }
    }
